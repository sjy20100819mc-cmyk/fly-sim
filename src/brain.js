...
const CFG = {
  ...
  food: {
    r: 15, emit: 5.2, feedRate: 22, feedRange: 4, odorSens: 0.9,
    energy: 12000,     // 一块新鲜食物的总能量（被取食即消耗）
    decay: 6,          // 自然变质损耗（能量/s）：无人吃也会慢慢消失
    maxFeeders: 4,     // 同时"满速取食"的只数，人多了每只都变慢（拥挤效应）
    consume: 1.0,      // 消耗系数：果蝇获得 1 能量 = 食物减少 consume
    minR: 0.42,        // 吃剩最后一点时的视觉最小半径比例
    stain: 22          // 吃完后残留痕迹的存留时间(s)
  },
  ...
  brain: {
    dt: 0.01,          // 仿真步长(s) 10ms
    stepsPerFrame: 1,  // 每帧最多推进的脑步数（实测接线更密，降为 1 保持流畅）
    wSyn: 0.275,       // 单突触权重基准 (mV)，flybench 默认值
    gain: 0.42,        // 全局增益：flybench 实测 0.40–0.45 才能通过全部反射
    K: 5,              // 权重尺度：w = w_syn·gain·K·(实测归一化强度)，按 flybench 反射判据标定
    adapt: 0.18,       // 放电频率适应强度（钙激活钾电流类比）：实测接线更密，调高以留出抑制空间
  ...
  fly: {
    ...
    reproChance: 0.12, reproCost: 50, reproEnergy: 108, feedNeed: 6, reproCool: 26,
...
const W = {
  ...
  foods: [], traps: [], eggs: [], corpses: [], flies: [], stains: [],
...
function compileNet() {
  ...
  for (const [a, b, norm, pl] of CONNECTIONS) {
    totalSyn += norm;
    const A = ct[a], Bq = ct[b];
    // 多巴胺是神经调质而非快速递质：DAN 的出边不产生突触电流，只门控可塑性
    const isMod = (a === 'PPL1' || a === 'PAM');
    if (isMod) modEdges++;
    /* 归一化强度 = log2(1 + 实测每后神经元突触数 / 中位数)，符号取该群递质性质 */
    const w = isMod ? 0 : B.wSyn * B.gain * B.K * norm * A.sign;
    for (let i = 0; i < A.n; i++) {
      for (let j = 0; j < Bq.n; j++) {
        S.push(A.start + i); D.push(Bq.start + j);
        W0.push(w); SYN.push(norm);
...
class Brain {
  constructor(gene) {
    ...
    this.daTrace = new Float32Array(NET.ct.PPL1.n);
    this.popRate = 0;
    this.popRateSlow = 0;
    this.lastSpikes = 0;
  ...
  step(dt) {
    ...
    const daP = this.rate('PPL1'), daR = this.rate('PAM');
  ...
  /* 转向偏置：实测连接组把左右腿运动神经元合并为一群（MN_walk），
     因此转向偏置改由中央复合体通路给出 —— PFN（桥神经元，扇形体/椭球体输出）
     相对 ring_EB（椭球体 ring，航向信号）的活跃度差。 */
  steerBias() { return (this.rate('PFN') - this.rate('ring_EB') * 0.4) / 25; }
...
class Fly {
  constructor(x, y, gene) {
    ...
    this.age = 0; this.eating = false; this.foodPatch = null; this.feedAcc = 0;
    this.state = 'cruise';
  ...
  encode(dt) {
    ...
    let onFood = false, patch = null;
    for (const f of W.foods) {
      if (Math.hypot(f.x - this.x, f.y - this.y) < CFG.food.r + CFG.food.feedRange) { onFood = true; patch = f; break; }
    }
    this.onFood = onFood;
    this.foodPatch = patch;
    ...
    this.setExt('PPL1', us + 1.5 * this.alSat * alW);
  ...
  update(dt) {
    ...
    this.eating = false;
    if (this.onFood && this.foodPatch && this.foodPatch.e > 0) {
      const feed = b.feedDrive();
      if (feed > F.feedThr) {
        const patch = this.foodPatch;
        const crowd = Math.min(1, CFG.food.maxFeeders / Math.max(1, patch.nFeeders || 1));  // 挤在一起就吃慢点
        const er = clamp(patch.e / patch.e0, 0, 1);
        let gain = CFG.food.feedRate * crowd * (0.35 + 0.65 * er) * dt;                     // 快见底时也吃得慢
        gain = Math.min(gain, patch.e / Math.max(0.05, CFG.food.consume));                  // 不能超过剩余量
        if (gain > 0) {
          this.eating = true;
          this.state = 'feed';
          patch.e -= gain * CFG.food.consume;                                               // 吃掉的部分从食物里扣
          this.energy = Math.min(F.eMax, this.energy + gain);
...
function place(x, y) {
  ...
  if (m === 'food') {
    ...
    const e = CFG.food.energy * rand(0.85, 1.15);
    W.foods.push({ x, y, ph: rand(0, TAU), e: e, e0: e, nFeeders: 0 });
    return true;
  }
  if (m === 'trap') {
...
function resetWorld() {
  W.foods = []; W.traps = []; W.eggs = []; W.corpses = []; W.flies = []; W.stains = [];
  W.deathsByCause = {};
  particles.length = 0;
  ...
  for (const [fx, fy, ph] of [[0.28, 0.35, 0], [0.7, 0.68, 1.7]]) {
    const e = CFG.food.energy * rand(0.9, 1.1);
    W.foods.push({ x: W.w * fx, y: W.h * fy, ph: ph, e: e, e0: e, nFeeders: 0 });
  }
}
...
function frame(ts) {
  ...
  if (typeof updateTip === 'function') updateTip();
  probeAcc += dt;
...
function simulate(dt) {
  W.time += dt;
  /* 每块食物统计同时取食的果蝇数（用上一帧的判定，平滑且够用） */
  for (const f of W.foods) f.nFeeders = 0;
  for (const fl of W.flies) if (!fl.dead && fl.onFood && fl.foodPatch && fl.foodPatch.e > 0) fl.foodPatch.nFeeders++;
  /* 气味强度与体积一起随剩余量衰减；并做自然变质 */
  for (let i = W.foods.length - 1; i >= 0; i--) {
    const f = W.foods[i];
    f.e -= CFG.food.decay * dt;
    const er = clamp(f.e / f.e0, 0, 1);
    fFood.deposit(f.x, f.y, CFG.food.emit * (0.25 + 0.75 * er) * dt, CFG.food.r * (0.7 + 0.6 * er));
    if (f.e <= 0) {                                  // 吃光/变质完了 → 消失，留个残迹
      W.stains.push({ x: f.x, y: f.y, t: 0, r: CFG.food.r * rand(0.8, 1.15), ph: rand(0, TAU) });
      if (W.stains.length > 40) W.stains.shift();
      sparks(f.x, f.y, [180, 150, 110], 7);
      for (const fl of W.flies) if (fl.foodPatch === f) { fl.foodPatch = null; fl.onFood = false; }
      W.foods.splice(i, 1);
      if (W.foods.length === 0 && !W.noFoodHinted) {
        W.noFoodHinted = true;
        if (typeof hint === 'function') hint('食物被吃光了 — 点 🍎 放新的食物');
      }
    }
  }
  if (W.foods.length > 0) W.noFoodHinted = false;
  for (let i = W.stains.length - 1; i >= 0; i--) { W.stains[i].t += dt; if (W.stains[i].t > CFG.food.stain) W.stains.splice(i, 1); }
...
function render(dt) {
  ...
  for (const f of W.flies) if (f.dead && !f.onTrap) drawFly(ctx, f);
...
function foodR(f) {                       // 食物视觉半径随剩余量缩小
  const er = clamp((f.e === undefined ? 1 : f.e / (f.e0 || 1)), 0, 1);
  return CFG.food.r * (CFG.food.minR + (1 - CFG.food.minR) * er);
}
function drawFood(ctx, f) {
  const r = foodR(f);
  const er = clamp((f.e === undefined ? 1 : f.e / (f.e0 || 1)), 0, 1);
  const g = ctx.createRadialGradient(f.x - r * 0.3, f.y - r * 0.3, 1, f.x, f.y, r);
  const mixc = (a, b2) => a.map((v, i) => Math.round(v + (b2[i] - v) * (1 - er)));
  const c0 = mixc([255, 217, 160], [170, 150, 120]);
  const c1 = mixc([227, 155, 78], [120, 96, 70]);
  const c2 = mixc([138, 75, 24], [70, 56, 44]);
  g.addColorStop(0, `rgb(${c0.join(',')})`);
  g.addColorStop(0.55, `rgb(${c1.join(',')})`);
...
function drawTrap(ctx, t) {
  ...
  if (t.bodies) for (const b of t.bodies) {
...
function drawEgg(ctx, e) {
  const t = e.t / e.ttl, pulse = 1 + Math.sin(W.time * 4 + e.ph) * 0.12;
  ctx.save();
  ...
  ctx.beginPath();
  for (let i = 0; i <= 12; i++) {
    const a = i / 12 * TAU, rr = 2.6 + Math.sin(a * 3 + W.time * 6) * 0.5;
...
function drawFly(ctx, f) {
  const F = CFG.fly, s = F.size * (f.gene.size || 1);
  ...
  } else if ((f.peakAversion || 0) > 0.35) {
    ctx.strokeStyle = 'rgba(140,180,255,0.45)'; ctx.lineWidth = 1;
    ctx.beginPath(); ctx.arc(f.x, f.y, 8.5, -0.5, 0.9); ctx.stroke();
    ctx.beginPath(); ctx.arc(f.x, f.y, 8.5, Math.PI - 0.5, Math.PI + 0.9); ctx.stroke();
  }
  if (W.showField) for (const m of f.mem) {
...
const LAY = (() => {
  const colOf = {
    ...
    PPL1: 3, PAM: 3, ring_EB: 3, PFN: 3, DN_escape: 3, DN_walk: 3,
    MN_feed: 4, MN_walk: 4, MN_oviposit: 4
  };
...
function uiTick(dt) {
  ...
  const foodE = W.foods.reduce((a, f) => a + Math.max(0, f.e), 0);
  const foodE0 = W.foods.reduce((a, f) => a + f.e0, 0) || 1;
  statsEl.innerHTML =
    ...
    ` <span class="k">(剩 ${(foodE / foodE0 * 100).toFixed(0)}%)</span>` +
...
const PARAM_SPEC = [
  ...
  ['brain.K', '权重尺度 K（实测结构标定）', 0.2, 6, 0.1],
  ...
  ['food.energy', '一块食物的总能量（吃光即消失）', 200, 20000, 100],
  ['food.decay', '自然变质损耗 (能量/s)', 0, 60, 0.5],
  ['food.maxFeeders', '同时满速取食的只数', 1, 20, 1],
  ['food.consume', '消耗系数（1能量 : 吃掉的量）', 0.1, 3, 0.05],
  ['g', '嗅觉 / 信息素'],
...
cv.addEventListener('pointerdown', e => {
  ...
  if (hitF) {
    W.inspect = hitF; W.tipSticky = true; showTipAt(hitF, p.x, p.y, true); down = false;
    hideHint(); return;
  }
  if (place(p.x, p.y) && W.mode === 'food') sparks(p.x, p.y, [255, 200, 120], 6);
...
cv.addEventListener('pointermove', e => {
  ...
  if (down) {
    ...
    const f = nearestFly(p.x, p.y);
    W.inspect = f;
    if (f) { if (!W.tipSticky) showTipAt(f, e.clientX, e.clientY, false); }
    else if (!W.tipSticky) hideTip();
  }
});
window.addEventListener('pointerup', () => { down = false; });
cv.addEventListener('pointerleave', () => { down = false; if (!W.tipSticky) { W.inspect = null; hideTip(); } });
...
let escapeCount = 0, escapeWindow = 0;
...
function hint(t) {
  ...
  el.textContent = t; el.style.opacity = 0.9;
  clearTimeout(el._t);
...
function reflexProbe() {
  ...
  r0.loomGF = run(() => set('mechano', 2.1)) >= 0 ? probeBrain.rate('DN_escape') : 0;
  ...
  r0.checks = [
    ...
    ['机械/听觉→巨纤维', r0.loomGF > 10, r0.loomGF.toFixed(0) + 'Hz'],
